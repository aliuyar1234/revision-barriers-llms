...
from src.data.rcicl_prompts import render_rcicl_prompt_samples
from src.data.rcicl_worlds import generate_rcicl_pairs, generate_rcicl_sanity
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate RC-ICL v1 worlds and sanity prompts.")
    ...
    pair_rows, pair_summaries = generate_rcicl_pairs(config)
    sanity_rows = generate_rcicl_sanity(config)
        ...
        samples.append(
            {
                ...
                "prompts": render_rcicl_prompt_samples(record),
            }
        )
        ...
        samples.append(
            {
                ...
                "history_type": record["history_type"],
                "prompts": {
                    "prefix_prompt": render_rcicl_prompt_samples(record)["prefix_prompt"],
                },
            }
        )
